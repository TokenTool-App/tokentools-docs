...
查看以下步骤，了解如何使用PC电脑端的MetaMask钱包通过TokenTool创建持币分红代币。

1. MetaMask钱包链接到区块链网络。
...
3. **拥有权转移：** 可以将owner权限转移给第三方地址 或者 其他地址
...
11. **最小触发数量：** 用户在swap收取到的手续费将会存在当前合约中，达到了`最小触发数量` 触发回流机制，会把合约中的本币兑换成`分红币`进行到分红，营销，流动性对应配置的手续费中
12. **销毁：** 销毁将币的数量进行销毁转入黑洞地址，转入到`0x000000000000000000000000000000000000dead` 地址。
13. **杀区块：** 自动杀区块,输入3代表杀3区块，意思就是前3区块(bsc大概9秒)买入的地址自动让代币转移到营销地址中。
14. **地址裂变：** 每笔交易都会自动向随机地址空投`一点点`代币,达到增加持币地址的效果
...
- **我创建的持币分红机制的代币，可以用`USDT`，`BUSDT` 或`自己的子币` 加池子吗?**
  - 解答：不可以，持币分红模板一定要用BNB池子。

-  **代码是自动开源的吗?**
	- 解答：是的,在区块浏览器设施完善的链上都是自动开源的,包括`ETH` `BSC` `ARB`…等等

- **营销进的是什么?**
  - 解答：营销钱包进的是BNB，因为持币分红V3机制是必须使用BNB加池子的，营销进的是BNB。
  
- **我在创建代币的时候，忘记打开黑名单开关，我还能用黑名单功能吗？**
	- 解答：不可以，如果你没打开黑名单功能，合约代码中将不会存在黑名单逻辑代码，检测合约机器人也不会检测到黑名单开关。 

- **上文说的 触发机制 是什么意思？**
  - 解答：非白名单地址在买卖时，收取的税会存放到合约地址中，有 **卖出操作** ,就会触发营销,回流，分红等机制

- **我在BSC链上，可以创建分红XX币，或者我自己创建的子币吗？**
  - 解答：只要你的XX币存在BNB交易对，就可以分红，主流币大部分都有BNB交易对，如`USDT` 、`BUSD` 、 `BTC` 、 `ETH ` 等等。
