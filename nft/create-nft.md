...
> **TokenTool是一个区块链工具箱，支持ETH、BSC等超多公链。**
...
2. 需要通过IPFS系统上传文件信息，并且生成JSON元数据（metadata）
...
6. **文件格式**：元数据文件后缀格式（通常为.json格式）


## NFT铸造信息
![image-20231204120545322](../.gitbook/assets/nft/Snipaste_2023-12-05_12-37-49.png)

1. **总供应量**：总共可铸造多少个NFT的总量。
1. **暂停Mint数量**：设置暂停铸造的触发数量。当铸造数量达到该值时，暂停铸造操作，设置0表示不暂停。（例如：总供应量5555，暂停数量设为5500，当铸造数量达到5500时暂停铸造，剩下55枚仅允许所有者铸造）
1. **Mint价格**：一个NFT的单价，价格只能是链币，设置0为免费铸造。（例如币安智能链BSC是BNB代币，以太坊链是ETH代币）
1. **免费铸造量**：免费铸造量，代表总NFT张数，有多少张NFT是免费的。（如例子假设设置10，总供应量5555，其中10个NFT是免费的，还有5545张NFT是收费的，根据Mint价格收费，如Mint价格设置0，则也代表是免费）。
1. **免费铸NFT限额**：每个用户最多免费铸造多少张NFT，(受”每个用户最大铸造数量”限制)，（如果”免费铸造量”已经超过则没有免费额度，按照Mint价格计算收费）。
1. **每个用户的最大铸NFT数**：每个地址最多可以铸造多少个 NFT（如设置10，代表每个地址最多铸造10个NFT）。
1. **每笔交易最大铸NFT数**：每笔交易最多铸造多少个NFT。
1. **开始时间/结束时间**：在设定的开始和结束时间内进行铸造NFT操作。
1. **白名单地址列表**：开启白名单后，只有白名单中的地址才有资格铸造NFT。
...
2. 例如访问[NFT铸造页面](https://tokentools.app/nft/mint/0x6974bCb700eDeDA5Cc1BfB0929c8D7e10607B470-56)，点击铸造按钮即可铸造。
...
### 常见问题解答
- **NFT能设置版税吗？**
  - 解答：可以的，版税正常都是在交易平台上设置，例如（OpenSea交易平台）ERC721标准NFT本身合约没有税。

- **IPFS文件系统是什么？**
  - 解答：可以理解成是一个去中心化的文件存储系统，详细资料信息可以通过[IPFS文件系统](https://ipfs.tech/)查看。
- **如何制作元数据链接？**
  - 解答：通过图片上传到IPFS后，再通过图片的IPFS的Hash值来生成元数据，再把元数据(json文件)再次上传到IPFS系统，具体教程可以点击[如何制作元数据链接教程](https://docs.tokentools.app/common-problem/generate-metadata)查看
- **元数据链接可以是中心化的地址吗？后期我还能更改吗？**
  - 解答：可以的，你可以通过自己的地址例如 `https://xxx.com/json/1.json`的形式，这种方式有点违背区块链去中心化常理。后期也可以在管理员页面进行修改元数据链接地址。
- **为什么更改了原数据链接交易平台图片没有变化？**
  - 解答：因为NFT交易平台都有图片缓存，刚更改的图片不会立马生效。你可以通过手动点击刷新（例如OpenSea平台）的 Refresh metadata 按钮刷新图片
