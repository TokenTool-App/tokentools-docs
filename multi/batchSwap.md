...
与大多数交易所相比，在 TokenTools Swap 上进行交易更加便捷。您无需担心滑点的复杂设置，系统会为您自动计算并提供最优结果。
...
**Q1：为什么要用私钥导入钱包？**
 ...
 答：因为批量操作需要直接控制钱包签名和交易，必须通过私钥导入。请确保私钥来源安全，避免泄露。

**Q2：导入的钱包有顺序吗？**

 答：有。默认是按照导入的顺序执行，但你也可以选择 **随机执行** 模式，打乱执行顺序。

**Q3：能不能用助记词导入？**

 答：目前仅支持私钥导入。如果需要助记词导入，建议先用工具转成私钥再导入。

**Q4：“从”和“到”是什么意思？**

 答：就是 Token0 兑换成 Token1。例如：从 BNB 到 USDT，表示用 BNB 兑换 USDT。

**Q5：支持多路径兑换吗？**

 答：根据选择DEX交易所，如果有提示不支持信息。必须选择单一交易对。比如 **USDT 交易对只能用 USDT 买卖，BNB 交易对只能用 BNB 买卖**。
...
**Q7：任务执行间隔有什么作用？**
 ...
 答：每次交易后会延迟一段时间再执行下一笔。支持固定间隔或随机间隔。
...
**Q10：自动卖出和买入的规则一样吗？**
 ...
 答：是的。自动卖出的金额规则（全部、随机、百分比、固定）与买入完全相同。

**Q11：必须开启自动卖出吗？**

 答：不是。自动卖出是可选功能，如果只想单向买入，可以不开启自动卖。

**Q12：运行次数是指什么？**

 答：是指导入的钱包循环执行多少轮。例如：运行次数设置为 2，则所有钱包执行两次交易。
...
**Q15：Gas Limit 应该怎么设置？**
 答：同样有固定、自动、随机三种模式。大部分情况下，建议用自动。
