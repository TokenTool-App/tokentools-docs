...
3. 释放周期/总发放：挖矿总天数和总的发放数量，如要计算每日发放量（总发放/总天数=每日发放量）
4. 如挖矿未开始，则会进入倒计时，等待挖矿开始后即可质押挖矿。
5. 邀请记录：用户通过您的邀请链接参与质押挖矿，将会显示其作为下级代理的信息以及您获得的邀请奖励信息。
...
1. 小狐狸/TP钱包dapp打开质押挖矿链接，链接钱包
2. 查看收益，点击领取
3. 收益产生规则：根据每天发放的数量，计算每秒钟的收益，根据质押占比，将会得到收益比例。例如（10天，发放1000个TT代币，每日发放 1000 / 10 = 100个TT，当前质押占比100%，那么当前用户每天能获取100个TT代币，100 / 86400秒 = 每秒钟可获得0.001157407407个TT代币）